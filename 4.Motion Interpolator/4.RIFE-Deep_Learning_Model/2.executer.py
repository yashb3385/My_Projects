...
def rgb_img_creater(channels_paths, output_path, img_max_pixel):
    ...
    rgb_image = cv2.resize(rgb_image, (h, w), interpolation=cv2.INTER_AREA)

    plt.imsave(output_path, rgb_image)
    print(f"      -> Successfully saved RGB composite with shape ({h} X {w}) to : {output_path}")
...
def single_channel_img_creater(file_path, output_image_path, color_map, img_max_pixel):
    ...
    if w > img_max_pixel:
        w = img_max_pixel
    rad_array = cv2.resize(rad_array, (h, w), interpolation=cv2.INTER_AREA)
    print(f"    Extracted image array with shape: ({h} X {w})")
...
def nc_imager(input_dir, output_dir, color_map, img_max_pixel, rgb_flag, chnl):
    ...
    if rgb_flag == None:
        ...
        if len(rgb_timestamps) > 0:
            print(f"\n[INFO] : Found all {len(rgb_timestamps)+1} channel sets capable of RGB rendering.")
    ...
    return user_choice, target_channel
...
def execute(path, gpu_flag):
    ...
    if nc_flag == 1 or png_flag == 1:
        ...
        if nc_flag == 1:
            ...
            RGB_flag, channel_no = nc_imager(path, f"{rife_path}/input", c_map, nc_img_max_size, None, None)
            if channel_no == "RGB_":
                channel = channel_no
            else:
                channel = f"C{int(channel_no):02d}_"
            if img_output_flag == 'y':
                nc_imager(path,f"2.output/output{input_pointer}/Native_{channel}Images" ,c_map, float('inf'), RGB_flag, channel_no)
        ...
        img_to_video(f"{rife_path}/input", f"{vid_output_path}/original_{channel}{int(fps)}fps.mp4", int(fps))
        img_to_video(f"{rife_path}/vid_out", f"{vid_output_path}/final_{channel}{2**n}X_{int((2**n)*fps)}fps.mp4", int((2**n)*fps))

        if img_output_flag == 'y':
            print("\n")
            for img in os.listdir(f"{rife_path}/vid_out"):
                move(f"{rife_path}/vid_out/{img}", f"{img_output_path}/{channel}{int(fps)}fps_to_{int((2**n)*fps)}fps_{2**n}X")
